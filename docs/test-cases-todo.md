...
7. ✅ **DONE** (`tests/integration/run_integration.sh`). **Auto-spawn refuses when the profile has `default_model: null`.** POST /v1/llama/designer-cpu/completion (no default_model in our shipped config) → 503 with "no default_model declared".
...
9. ✅ **DONE** (`tests/gguf_test.hml`). **Mistral-Nemo metadata lie.** `claimed_context(parsed)` returns 1024000, `safe_context(parsed)` returns 131072 (override applied). Synthetic header-only fixture `tests/fixtures/nemo_llama.gguf` (generated by `make_gguf_fixtures.py`, no tensor data).

10. ✅ **DONE** (`tests/integration/run_integration.sh`). **CPU-only nodes report `role: "cpu"` and `gpus: []`.** Mock `nvidia-smi` returning non-zero (or simply not on PATH) → `hardware.probe()` returns `{role: "cpu", gpus: []}` cleanly, no exception.

11. ✅ **DONE** (`tests/gguf_test.hml`). **`max_context_tokens` caps at `safe_context`.** A model with claimed=1024K, safe=128K, asked for max ctx on a card that COULD fit far more KV → returns 131072, not the larger window. Also covers the 0-return when weights+overhead exceed VRAM.

12. ✅ **DONE** (`tests/gguf_test.hml`). **Bad GGUF magic throws clear error.** `bad_magic.bin` → throws `"not a GGUF file"`; `gguf_v2.gguf` → throws unsupported-version. Both via `assert_throws`.
