...
- **SSE streaming** — ✅ done. The `/v1/llama/{profile}/*` proxy detects `stream:true`, opens an upstream `http_stream` to the llama-server, and pumps its `text/event-stream` chunks straight through (`http_server.write_response` reads `read_binary` until the source closes). Verified streaming token-by-token through the CP in prod. (flammen itself stays all-async and doesn't use it; it's there for streaming consumers.)
